...
def prepare(
    ...
    *,
    ...
    ewm_halflife        applied to BOTH X and y. The project default of 120min is
                        only ~2.3x the median reel spacing (52min), giving about 7
                        effective samples and removing ~10% of the target
                        variance. Variance removal plateaus near 13% at an 8-12h
                        halflife, so this is worth sweeping rather than assuming.
    """
    def log(*a):
        if verbose:
            print(*a)
    ...
    if apply_ewm_filter_y:
        # Same segmentation and halflife as the X filter, but written out here so
        # the halflife is configurable. utility.ewm_reset hardcodes 120min, which
        # is only ~2.3x the median reel spacing and removes just ~10% of the
        # target variance.
        grammage_group = pd.Series(turnup_ts["grammage"].values)
        grade_change = grammage_group.ne(grammage_group.shift())
        time_gap = ts_transformed.index.to_series().diff().gt(
            pd.Timedelta(ewm_gap_threshold))
        ...
        def _ewm_seg(s):
            out = s.ewm(halflife=ewm_halflife, times=s.index, adjust=True).mean()
            if len(s) >= 2:
                out.iloc[0] = (s.iloc[0] + s.iloc[1]) / 2.0
            return out

        ts_transformed[y_column] = y_series.groupby(seg).transform(_ewm_seg).values
