...
_PLS_REGISTRY = {
    ...
    "linepressure": (LINEPRESSURE_VARS, 2),
    "conc_starch": (CONC_STARCH_VARS, 1),
...
def build_pre_estimator(
    ...
    # Ensure created variables are in selected_model_features so they pass
    # through FeatureCreator (they were excluded from exog_vars_reduced upstream
    # to avoid treating them as raw inputs, but they must still appear as model
    # features after creation).
    selected_model_features = unique_in_order(selected_model_features + created_vars)
    ...
    for _, step in prep_pip.steps:
        ...
        if selected_transformed:
            ...
            # Also remove PLS source columns from selected_model_features
            # (they are consumed by the PLS step)
            selected_model_features = ordered_difference(
                selected_model_features, source_names
            )
            ...
            # Remove PLS source columns from filter (they get consumed by the
            # PLS step and replaced by scores)
            features_to_keep_filter = ordered_difference(
                features_to_keep_filter, source_names
            )
    ...
    # Determine which created variables are needed: either as model features
    # or as PLS source columns (they appear in features_to_keep_feat)
    features_to_create = ordered_intersection(created_vars, features_to_keep_feat)
