...
def main():
    ...
    parser.add_argument("--protocol_compare", action="store_true",
                        help="Score identical data under terminal-split vs block-CV, "
                             "with and without partialling, to separate the method "
    ...
    # Protocol comparison
    # =========================================================================
    # Separates "the method" from "how it was scored". A single terminal 80/20
    # block and contiguous k-fold CV can differ enormously on drifting data: the
    # terminal block is one particular regime, whereas CV averages over several
    # and includes edge blocks. Without this, a change in R2 cannot be attributed
    # to the model at all.
    protocol_df = pd.DataFrame()
    if args.protocol_compare:
        print("\n--- Protocol comparison (identical data and features) ---")
        from sklearn.metrics import r2_score as _r2s

        Xv_all = X.values.astype(float)
        pdat_pc = P.partial_out_time(X, y, t_hours, best_bw_h)
        split_term = int(len(y) * 0.80)
        tr_t = np.arange(split_term)
        va_t = np.arange(split_term, len(y))
        make_r = P.LEARNERS["ridge"]()
        blocks = P.contiguous_blocks(len(y), N_FOLDS_RUN)

        def _terminal(Xm, ym):
            m = make_r()
            m.fit(Xm[tr_t], ym[tr_t])
            return float(_r2s(ym[va_t], m.predict(Xm[va_t]))), None
        ...
        def _blockcv(Xm, ym):
            ...
            for a, b in blocks:
                m = make_r()
                m.fit(Xm[a], ym[a])
                sc.append(float(_r2s(ym[b], m.predict(Xm[b]))))
            return float(np.mean(sc)), sc

        proto_rows = []
        for tgt_name, Xm, ym in [
            ("raw target (drift left in)", Xv_all, y),
            (f"partialled y~ (bw={best_bw_h/24:.2f}d)", pdat_pc.X_tilde, pdat_pc.y_tilde),
        ]:
            for proto_name, fn in [("terminal 80/20", _terminal),
                                   (f"{N_FOLDS_RUN}-fold block CV", _blockcv)]:
                score, per_fold = fn(Xm, ym)
                proto_rows.append({
                    "target": tgt_name,
                    "protocol": proto_name,
                    "r2": score,
                    "per_fold": ("" if per_fold is None
                                 else " ".join(f"{v:+.3f}" for v in per_fold)),
                })
        protocol_df = pd.DataFrame(proto_rows)
        print(protocol_df.to_string(index=False, float_format=lambda v: f"{v:+.4f}"))
        print("  No feature selection anywhere, so none of these numbers are")
        print("  inflated by choosing features or iterations on the scored block.")
    ...
    if not protocol_df.empty:
        protocol_df.to_csv(out_dir / "protocol_comparison.csv", index=False)
